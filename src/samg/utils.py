...
def _load_places(dataset_dir, batch_size=256, image_size=84, num_workers=4, use_val=False):
    ...
    assert os.path.exists(dataset_dir), f"place365 dataset not found at the specified path {dataset_dir}"
    fp = os.path.join(dataset_dir, partition)
...
def random_overlay(x, dataset_dir, dataset='places365_standard'):
